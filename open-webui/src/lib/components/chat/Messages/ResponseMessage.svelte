<script lang="ts">
	...
	const renderStyling = async () => {
		...
		// 	if (message.content.includes(d.left) && message.content.includes(d.right)) {
		// 		renderLatex();
		// 		return;
		// 	}
		// });
					...
					<div class="my-2.5 w-full flex overflow-x-auto gap-2 flex-wrap">
						{#each message.files as file}
							<div>
								{#if file.type === 'image'}
									<Image src={file.url} />
									<div style="font-size: 0.75rem; white-space: nowrap; display: inline-flex; align-items: center; padding-top: 6px">
										<span style="vertical-align: middle;">
											<Warning />
										</span>
										<strong style="vertical-align: middle; margin-left: 1px; font-weight: 400; color: #FFCC4D">
											Follow established release approval process for external use.
										</strong>
									</div>
