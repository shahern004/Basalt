		...
		<div class=" mt-2 mb-5 text-2xl text-gray-800 dark:text-gray-100 font-semibold">
			...
			{:else}
				<div class=" line-clamp-1">{$i18n.t('Hello, {{name}}!', { name: $user.name })}</div>
					<br/>
				<div class="intro">{$i18n.t("Ask me a question with or without document uploads and I \
				                             will respond to the best of my knowledge. If you prefer, we \
											 can speak instead of typing to each other.")}</div>
			{/if}
		</div>
	</div>
{/if}
