	...
	onMount(async () => {
		...
		const onResize = () => {
			if(show && window.innerWidth < BREAKPOINT) {
				show = false;
			} else if (!show && window.innerWidth >= BREAKPOINT) {
				show = true;
			}
		}
...
<div
	...
	class="max-height  {show
		? 'lg:relative w-[260px]'
		: '-translate-x-[260px] w-[0px]'}  bg-black text-gray-200 shadow-2xl text-sm transition z-40 fixed left-0
        "
>
	<div
		class="py-2.5 my-auto flex flex-col justify-between max-height w-[260px] {show
			? ''
			: 'invisible'}"
	>
				...
				{#if $user !== undefined}
					...
					{#if showDropdown}
						...
						>
							...
							<!-- Disabled 'Sign Out' to avoid conflicts with Basalt Authentication flow -->
